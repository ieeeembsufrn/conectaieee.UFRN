-- db_schema_ufrn.sql
-- Copia do schema base para implantacao na Universidade Federal do Rio Grande do Norte.
-- O schema e generico; os dados especificos do ramo ficam em db_seed_ufrn.sql.
...
create table public.profiles (
  ...
  email text not null,
	...
	-- 3.2 Tokens de notificação por dispositivo
	create table public.notification_tokens (
	  ...
	  notify_due_tasks boolean not null default true,
	  notify_overdue_tasks boolean not null default true,
	  notify_chapter_events boolean not null default true,
	...
	create index idx_notification_tokens_profile_id on public.notification_tokens(profile_id);
	create index idx_notification_tokens_enabled on public.notification_tokens(enabled);
	create index idx_notification_tokens_active_profile_id on public.notification_tokens(profile_id) where enabled = true;
	create index idx_notification_tokens_active_platform on public.notification_tokens(platform) where enabled = true;
